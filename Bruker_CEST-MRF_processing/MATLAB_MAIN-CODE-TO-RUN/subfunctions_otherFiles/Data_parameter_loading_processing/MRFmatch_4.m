...
%
function MRFmatch_4(dirstruct,prefs,PV360flg)
...
if isfield(dirstruct,'use_cluster') && dirstruct.use_cluster
    ...
    env_vars = sprintf('USE_CNN=%d,NUM_GPUS=%d', use_cnn_flag, dirstruct.num_gpus);

    if isfield(dirstruct,'cluster_large_storage_dir') && ~isempty(dirstruct.cluster_large_storage_dir)
        fprintf('  Large storage: %s\n', dirstruct.cluster_large_storage_dir);
        env_vars = sprintf('%s,LARGE_STORAGE_DIR=%s', env_vars, dirstruct.cluster_large_storage_dir);
    end

    % Construct sbatch command
    sbatch_cmd = sprintf('cd %s && sbatch --export=ALL,%s %s', ...
        dirstruct.cluster_dir, ...
        env_vars, ...
        job_script);

    % Submit via SSH (with proper quoting)
    ssh_cmd = sprintf('ssh -o LogLevel=QUIET %s@%s ''%s''', ...
        dirstruct.cluster_user, ...
        dirstruct.cluster_host, ...
        sbatch_cmd);
    ...
    if status ~= 0
        ...
        fprintf('Output: %s\n', result);
        fprintf('========================\n');
        error('Failed to submit cluster job');
    end

    % Extract job ID from sbatch output
    job_id_match = regexp(result, 'Submitted batch job (\d+)', 'tokens');

    if isempty(job_id_match)
        fprintf('\n=== Failed to find job ID ===\n');
        fprintf('Command: %s\n', ssh_cmd);
        fprintf('Output:\n%s\n', result);
        fprintf('===========================\n');
